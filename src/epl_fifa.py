...
df = pd.read_csv(os.path.join('./data','Premier_League.csv'),index_col=0)
...
data = df.copy()
df_new = df.copy()
df_new = df_new[df_new.year>'2012-13']
...
for i in range(15,20,1):
  ...
  fifaPlayerRatings = pd.read_csv('data/players_{}.csv'.format(toYear))

  bundesligaPlayersFifaRating = fifaPlayerRatings
  ...
  skySportsToFifa['QPR'] = 'Queens Park Rangers'
  fifaToSkySports['Queens Park Rangers'] = 'QPR'

  skySportsToFifa["M'boro"] = 'Middlesbrough'
  fifaToSkySports['Middlesbrough'] = "M'boro"

  skySportsToFifa['W Brom'] = 'West Bromwich Albion'
  fifaToSkySports['West Bromwich Albion'] = 'W Brom'

  skySportsToFifa['Man Utd'] = 'Manchester United'
  fifaToSkySports['Manchester United'] = 'Man Utd'

  skySportsToFifa["So'ton"] = 'Southampton'
  fifaToSkySports['Southampton'] = "So'ton"

  skySportsToFifa["B'mouth"] = 'Bournemouth'
  fifaToSkySports['Bournemouth'] = "B'mouth"

  skySportsToFifa["Huddsf'ld"] = 'Huddersfield Town'
  fifaToSkySports['Huddersfield Town'] = "Huddsf'ld"
  
  skySportsToFifa['Wolves'] = 'Wolverhampton Wanderers'
  fifaToSkySports['Wolverhampton Wanderers'] = 'Wolves'

  skySportsToFifa['Sheff Utd'] = 'Sheffield United'
  fifaToSkySports['Sheffield United'] = 'Sheff Utd'
...
df_train = df_train.drop(['date', 'venue', 'attendance'],axis=1)
df_test = df_test.drop(['date', 'venue', 'attendance'],axis=1)
...
if args.model=='rf':
  ...
  clf = RandomForestClassifier(random_state=42,n_estimators=135,max_depth=10,min_samples_split=8)
  clf.fit(X_train, y_train)
  y_pred = clf.predict(X_test)
